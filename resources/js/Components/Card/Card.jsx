...
export default function Card({ id, name, img, desc, href, quantity, addSelectedItems, minusSelectedItems, price, stock}) {
    ...
    return (
        <div id={`${name}`} className="flex md:border md:rounded-lg md:py-4 md:px-6 z-0">
            <div className="relative w-[92px] md:w-[172px] cursor-pointer text-center">
                {/* <img src="" alt="" /> */}
                <div className="absolute top-2 left-2 bg-white rounded-full p-1.5 cursor-pointer" onClick={handleHeartClick}>
                    {isLiked ? (
                ...
                (
                    <div className="flex self-end">
                        <button onClick={handleRemoveFromSelectedItems} disabled={quantity === 0} className="bg-[#42754c49] text-gray-600 hover:bg-gray-300 rounded-full px-2.5">-</button>
                            ...
                            <button onClick={handleAddToSelectedItems} disabled={quantity === stock} className="bg-[#42754c49] text-gray-600 hover:bg-gray-300 rounded-full px-2.5">+</button>
                        </div>
                ) : (
